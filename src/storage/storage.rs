    ...
    pub fn new(directory: String, capacity: usize) -> Self {
        ...
        let bundles = if file_path.exists() {
            match fs::read_to_string(&file_path) {
                // si le fichier eciste on charge les bundles
                // essaie de lire le fichier en entier comme texte !!!!!!!  et retourne le content
                Ok(content) => {
                    match serde_json::from_str::<Value>(&content) {
                        // si ok on parse le json en structure value qui est un arbre generique
                        Ok(json_value) => {
                            // nouveau match car la serialisation peut echouer
                            if let Some(bundles_array) =
                                json_value.get("bundles").and_then(|v| v.as_array())
                            {
                                bundles_array
                                    .iter()
...
=======
    // Issue #11
    // Duplicate detection logic
>>>>>>> 8173500 (fix after push correction)
